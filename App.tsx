...
// Version 2.0.0 - Production Ready (Multi-compte + Undo + Cloud)
...
const App = () => {
  ...
  // Récupérer l'ID de compagnie pour l'affichage
  const configCompanyId =
  (localStorage.getItem("crewflo_company_id") || "").trim();
...
const canEdit = role === 'admin';
  ...
  useEffect(() => {
    // 1. Check for Android/Desktop install prompt
  ...
  // Utilisation du Sync Store Hybride avec status et undo
  ...
  const handleGlobalUndo = () => {
    // Trouver quelle base de données a été modifiée en dernier
    ...
    if (maxMod === 0) return; // Rien à annuler

    // Priorité à l'action la plus récente
    if (canUndoT && lastModT === maxMod) {
        undoT();
    } else if (canUndoP && lastModP === maxMod) {
        undoP();
    } else if (canUndoS && lastModS === maxMod) {
        undoS();
    }
  };
  ...
  return (
          <CalendarView 
            canEdit={canEdit} role={role} projects={projects}
            suppliers={suppliers}
            tasks={tasks}
            setTasks={setTasks}
  ...
  const StatusIndicator = () => {
    ...
    if (globalStatus === 'saving') {
        return <div className="flex items-center gap-2 text-yellow-500 text-[10px] font-bold animate-pulse"><Loader2 className="w-3 h-3 animate-spin" /> Envoi...</div>;
    }
    if (globalStatus === 'saved') {
        return <div className="flex items-center gap-2 text-green-500 text-[10px] font-bold"><CheckCircle2 className="w-3 h-3" /> Synchronisé</div>;
    }
    if (globalStatus === 'error') {
        return <div className="flex items-center gap-2 text-orange-500 text-[10px] font-bold" title="Pas de connexion internet. Sauvegardé sur l'appareil."><AlertTriangle className="w-3 h-3" /> Hors ligne (Local)</div>;
    }
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
          ...
          <div className="bg-white w-full max-w-sm rounded-xl p-6 shadow-2xl relative animate-in slide-in-from-bottom">
            ...
            <ol className="space-y-4 text-sm text-slate-600">
              <li className="flex gap-3">
                ...
                <div>1. Appuyez sur le bouton <strong>Partager</strong> dans la barre du navigateur (souvent en bas).</div>
              </li>
              <li className="flex gap-3">
                <div className="flex-none bg-slate-100 p-2 rounded-lg h-fit"><PlusSquare className="w-5 h-5 text-slate-600" /></div>
                <div>2. Cherchez et appuyez sur <strong>Sur l'écran d'accueil</strong>.</div>
              </li>
              <li className="flex gap-3">
      ...
      `}>
        ...
        {/* Global Undo Button in Sidebar (Visible Desktop/Mobile menu) */}
        {canGlobalUndo && (
            <div className="px-4 pt-4 animate-in slide-in-from-left">
                <button 
                    onClick={handleGlobalUndo}
                    className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-sm font-bold bg-orange-600 text-white hover:bg-orange-700 transition-colors shadow-lg shadow-orange-900/20"
                >
        ...
        <nav className="p-4 space-y-2 flex-1 overflow-y-auto">
          <button 
            onClick={() => { setCurrentView('calendar'); setSelectedProjectId(null); setIsSidebarOpen(false); }}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${currentView === 'calendar' && !selectedProjectId ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
          >
          ...
          {projects.map(p => (
            <button
              key={p.id}
              onClick={() => { setCurrentView('calendar'); setSelectedProjectId(p.id); setIsSidebarOpen(false); }}
              className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${currentView === 'calendar' && selectedProjectId === p.id ? 'bg-blue-900/50 text-blue-200 border border-blue-800' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
            >
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${currentView === 'projects' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
          >
          ...
          <button 
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${currentView === 'suppliers' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
          >
          ...
          <button 
            onClick={() => { setCurrentView('ai'); setIsSidebarOpen(false); }}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${currentView === 'ai' ? 'bg-gradient-to-r from-purple-600 to-blue-600 text-white shadow-lg' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
          >
        ...
        <div className="p-4 border-t border-slate-800 space-y-3 flex-none bg-slate-900/50">
           ...
           {/* BOUTON INSTALLATION ANDROID/PC */}
           {!isStandalone && deferredPrompt && (
             <button 
                onClick={handleInstallClick}
                className="w-full flex items-center gap-2 px-3 py-2.5 mb-2 rounded text-xs font-bold transition-colors bg-blue-600 text-white hover:bg-blue-500 animate-in slide-in-from-left"
             >
           ...
           <button 
              ...
              className={`w-full flex items-center gap-2 px-3 py-2.5 rounded text-xs font-bold transition-colors border ${isCloudConnected ? 'bg-green-900/20 text-green-400 border-green-900 hover:bg-green-900/40' : 'bg-slate-800 text-slate-300 border-slate-700 hover:bg-slate-700'}`}
           >
           ...
           <div className="grid grid-cols-2 gap-2">
              <button 
                  onClick={exportData}
                  className="flex flex-col items-center justify-center p-2 bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white rounded transition-colors"
                  title="Exporter"
              >
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="lg:hidden bg-white border-b border-slate-200 p-4 flex items-center justify-between">
           ...
           {/* Bouton Undo Mobile dans le header s'il y a quelque chose à annuler */}
           {canGlobalUndo && (
             <button 
               onClick={handleGlobalUndo}
